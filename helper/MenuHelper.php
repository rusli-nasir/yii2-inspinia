...
namespace dxapp\themes\inspinia\helper;
...
//        $config = Configs::instance();
        ...
        $manager = \Yii::$app->authManager;//Configs::authManager();
        ...
        if ($refresh || $cache === null || ($assigned = $cache->get($key)) === false) {
            ...
            if ($cache !== null) {
                $cache->set($key, $assigned, self::CACHE_DURATION, new TagDependency([
                    'tags' => self::CACHE_TAG,// Configs::CACHE_TAG
                ]));
            }
        }
